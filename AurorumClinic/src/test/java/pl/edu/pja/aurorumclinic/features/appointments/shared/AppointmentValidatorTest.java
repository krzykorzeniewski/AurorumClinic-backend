...
public class AppointmentValidatorTest {
    ...
    void validateAppointmentShouldThrowApiExceptionWhenStartDateIsAfterEndDate() {
        LocalDateTime startedAt = LocalDateTime.now().plusMinutes(60);
        LocalDateTime finishedAt = LocalDateTime.now().plusMinutes(30);

        Specialization testSpec = Specialization.builder()
                .id(1L)
                .name("Psychiatra dorosłych")
                .services(Set.of(Service.builder().id(1L).build()))
                .build();

        Service testService = Service.builder().id(2L).build();
    ...
    void validateAppointmentShouldThrowApiExceptionWhenEndDateIsBeforeStartDate() {
        ...
        LocalDateTime finishedAt = LocalDateTime.now().plusMinutes(10);
    ...
    void validateAppointmentShouldThrowApiExceptionWhenTimeslotIsNotAvailable() {
        ...
        Service testService = Service.builder().id(1L).build();
    ...
    void validateRescheduledAppointmentShouldThrowApiExceptionWhenDoctorSpecializationIsNotAssignedToService() {
        ...
        Service requestedService = Service.builder().id(2L).build();
        ...
        assertThatThrownBy(() ->
                appointmentValidator.validateRescheduledAppointment(startedAt, finishedAt, testDoctor, requestedService, testAppointment)
        ).isExactlyInstanceOf(ApiException.class)
                .message().containsIgnoringCase("specialization");
    }
    ...
    void validateRescheduledAppointmentShouldThrowApiExceptionWhenTimeslotIsNotAvailable() {
        ...
        assertThatThrownBy(() ->
                appointmentValidator.validateRescheduledAppointment(startedAt, finishedAt, testDoctor, testService, testAppointment)
        ).isExactlyInstanceOf(ApiException.class)
                .message().containsIgnoringCase("timeslot");

        verify(appointmentRepository).isTimeSlotAvailableExcludingId(
                startedAt, finishedAt, testDoctor.getId(), testService.getId(), testAppointment.getId()
        );
    }
